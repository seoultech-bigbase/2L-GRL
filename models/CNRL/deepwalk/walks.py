...
def _write_walks_to_disk(args):
  num_paths, path_length, alpha, rand, f, rw_method, vector, cmty = args
  G = __current_graph
  t_0 = time()
  print('alpha in _write', alpha)
  print('cmty', cmty)
  with open(f, 'w') as fout:
    for walk in graph.build_deepwalk_corpus_iter(G=G, num_paths=num_paths, path_length=path_length,
                                                 alpha=alpha, rand=rand, rw_method=rw_method, vector=vector, cmty=cmty):
...
def write_walks_to_disk(G, filebase, num_paths, path_length, alpha=0, rand=random.Random(0), num_workers=cpu_count(),
                        always_rebuild=True, rw_method=0, vector=[], cmty=[]):
  ...
  args_list = []
  files = []
  # print 'vector in write_walks_to_disk'
  ...
  with ProcessPoolExecutor(max_workers=num_workers) as executor:
    for size, file_, ppw in zip(executor.map(count_lines, files_list), files_list, paths_per_worker):
      if always_rebuild or size != (ppw*expected_size):
        args_list.append((ppw, path_length, alpha, random.Random(rand.randint(0, 2**31)), file_, rw_method, vector, cmty))
      else:
        files.append(file_)
