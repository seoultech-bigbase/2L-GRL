...
def train_sentence_topic(model, sentence, alpha, _work):
    ...
    cdef unsigned long long table_len
    cdef unsigned long long next_random
    cdef REAL_t *cmty_count = <REAL_t *> (np.PyArray_DATA(model.cmty_count))
    cdef int cmty_num = model.cmty_num
    if hs:
        syn1 = <REAL_t *>(np.PyArray_DATA(model.syn1))
    ...
    with nogil:
        for i in range(sentence_len):
            ...
            for j in range(j, k):
                if j == i or codelens[j] == 0:
                    continue
                cmty_count[indexes[j]*cmty_num+topic[j]] += 1
                if hs:
...
# by Anthony, use sentence
def train_sentence_topic2(model, sentence, alpha, _work):
    cdef int hs = model.hs
    cdef int negative = model.negative
    ...
    cdef int cmty_num = <int>model.cmty_num
    cdef int i, j, k
    cdef long result = 0
    cdef unsigned long long row1, row2

    # For hierarchical softmax
    cdef REAL_t *syn1
    cdef REAL_t *sentence_embedding = <REAL_t *>(np.PyArray_DATA(model.sentence_embedding))
    cdef REAL_t *context_embedding = <REAL_t *>(np.PyArray_DATA(model.context_embedding))
    cdef np.uint32_t *points[MAX_SENTENCE_LEN]
    cdef np.uint8_t *codes[MAX_SENTENCE_LEN]
    ...
    for i in range(sentence_len):
        ...
        if word is None:
            ...
            result += 1
    cdef np.uint32_t topic[MAX_SENTENCE_LEN]
    cdef REAL_t dot_res = <REAL_t>0.0
    cdef REAL_t sumexp = <REAL_t>0.0

    cdef REAL_t* nwsum = <REAL_t *>(np.PyArray_DATA(model.nwsum))
    cdef REAL_t* cmty_count = <REAL_t *>(np.PyArray_DATA(model.cmty_count))
    cdef REAL_t* mulp = <REAL_t *>(np.PyArray_DATA(model.mulp))

    cdef int lenvoc = len(model.vocab)
    cdef REAL_t p1
    cdef REAL_t beta = <REAL_t>(0.5)
    cdef REAL_t summulp = <REAL_t>(0.0)
    cdef REAL_t random_max = <REAL_t>RAND_MAX
    cdef int random_count = 0
    cdef REAL_t random_number
    cdef int cmty_choosen
    #topic = [item[1] for item in sentence]

    # release GIL & train on the sentence
    with nogil:
        # get the sentence_embedding
        for j in range(size):
            sentence_embedding[j] = 0
        for i in range(sentence_len):
            for j in range(size):
                sentence_embedding[j] += syn0[indexes[i]*size+j]
        ...
        for i in range(sentence_len):
            ...
            for j in range(j, k):
                ...
                row1 = indexes[j]*size
                row2 = indexes[j]*<int>cmty_num
                for ind in range(size):
                    context_embedding[ind] = (sentence_embedding[ind] - syn0[row1+<int>ind])/(sentence_len-1)

                summulp = <REAL_t>(0.0)
                for cmty in range(cmty_num):
                    mulp[<int>cmty] = <REAL_t>(0.0)
                for cmty in range(cmty_num):
                    p1 = (cmty_count[row2+<int>cmty]+beta)/(nwsum[<int>cmty]+lenvoc*beta)
                    dot_res = <REAL_t>(0.0)
                    for ind in range(size):
                        dot_res += context_embedding[ind]*syn0_topic[<int>cmty * size+<int>ind]
                    mulp[<int>cmty] = p1*exp(<REAL_t>dot_res)
                    summulp += mulp[<int>cmty]

                # normalize mulp
                for cmty in range(cmty_num):
                    mulp[<int>cmty] /= summulp

                for cmty in range(1, cmty_num):
                    mulp[<int>cmty] += mulp[<int>cmty-1]

                random_number = rand()/random_max
                for cmty_choosen in range(cmty_num):
                    if random_number < mulp[<int>cmty_choosen]:
                        break
                cmty_count[row2+<int>cmty_choosen] += 1
                nwsum[<int>cmty_choosen] += 1

                if hs:
                    fast_sentence_topic(points[i], codes[i], codelens[i], syn0, syn1, syn0_topic, size, cmty_choosen, _alpha, work)
...
# by Anthony, use window instead of sentence 
def train_sentence_topic4(model, sentence, alpha, _work):
    cdef int hs = model.hs
    cdef int negative = model.negative
    ...
    with nogil:
        ...
        for i in range(sentence_len):
            ...
            for jj in range(j, k):
                for ind in range(size):
                    context_embedding[ind] += syn0[indexes[jj]*size+<int>ind]
            for j in range(j, k):
                if j == i or codelens[j] == 0:
                    continue
                ...
                for cmty in range(cmty_num):
                    mulp[<int>cmty] = <REAL_t>(0.0)
                for cmty in range(cmty_num):
                    p1 = (cmty_count[indexes[j]*<int>cmty_num+<int>cmty]+beta)/(nwsum[<int>cmty]+lenvoc*beta)
                    row = <int>cmty * size
                    dot_res = <REAL_t>(0.0)
                    for ind in range(size):
                        dot_res += context_embedding[ind]*syn0_topic[row+<int>ind]
                    mulp[<int>cmty] = p1*exp(<REAL_t>dot_res)
                    summulp += mulp[<int>cmty]
                ...
                random_number = rand()/random_max
                for k in range(cmty_num):
                    if random_number < mulp[<int>k]:
                        break
                cmty_count[indexes[j]*<int>cmty_num+<int>k] += 1
                nwsum[<int>k] += 1

                if hs:
                    fast_sentence_topic(points[i], codes[i], codelens[i], syn0, syn1, syn0_topic, size, k, _alpha, work)
